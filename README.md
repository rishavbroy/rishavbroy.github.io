...
Use project mentor fields deliberately: a `Research note` status communicates scope, while the mentor list communicates provenance or advising context. Project date ranges are stored as discrete academic terms, using `YYYY-spring`, `YYYY-summer`, `YYYY-fall`, or `present`. Project tags should be standard, reusable labels; avoid near-duplicates such as `Development` and `Development Economics`, and prefer the more specific shared label when one exists. Project links may use plain URL values or `{ href, label }` objects when a link needs a precise public label such as `Presentation (May 2025)` or `Draft`.
