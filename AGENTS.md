...
These notes are for coding agents and review assistants working on this repository. Keep the README focused on setup, build, CV, and deployment tasks.

## Content data

Research projects are Markdown entries because they may later become full project pages. Until a route renders Markdown bodies, avoid repeated placeholder prose below the frontmatter. Courses are stored in `src/data/courses.json` because they are structured records rather than essays. Professors and mentors are referenced by stable IDs and resolved through `src/data/people.ts`, so one name or link update affects every card that cites that person. People without stable public pages may omit `url`; their names render as plain text instead of links. Both projects and courses are Astro content collections with Zod schemas, which provides build-time validation and typed `getCollection()` access.
...
Only active fonts should be registered in `astro.config.mjs` and rendered with `<Font />` in `src/layouts/BaseLayout.astro`. Inactive font experiments may remain commented out beside the active font entries, but the active config, layout tags, and `src/styles/tokens.css` values should agree. Social profile icons are local inline SVGs in `src/components/SocialLinks.astro`; do not add an icon dependency for one or two static brand links unless the site needs a broader icon system. The favicon assets are in `public/images/` and are referenced from `BaseLayout.astro`; keep those URLs stable because search engines can use them in search results. Structured data helpers are in `src/lib/structuredData.ts`. The homepage emits `WebSite`, `Person`, and `ProfilePage` nodes; list pages emit `WebSite`, `Person`, and `CollectionPage` nodes. Keep the `sameAs` profile links in the `Person` node consistent with the visible GitHub and LinkedIn links. Do not add placeholder structured-data fields to production pages; leave unknown facts out until they can be filled accurately. The static `public/sitemap.xml` and `public/robots.txt` files list the public pages and CV PDF intended for search discovery.

## Filtering and search

The Research and Courses pages use progressively enhanced client-side filtering. Cards are rendered at build time, so all content remains visible if JavaScript is disabled. When JavaScript is available, `CollectionFilters.astro` reads `data-*` attributes on each card and applies search plus multi-select filters. Within a filter group, selected values use OR logic; across filter groups, groups use AND logic. The Research date filter uses two overlaid native range inputs with integer `step` values corresponding to discrete academic-term points; the visual track highlights the selected interval between the two handles. The `In progress` research status filter is derived from project date ranges ending in `present`, so it can coexist with more specific statuses such as `Research paper`.
